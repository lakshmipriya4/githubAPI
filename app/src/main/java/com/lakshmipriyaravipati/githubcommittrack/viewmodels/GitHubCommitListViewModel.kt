...
class GitHubCommitListViewModel @Inject constructor(var itemsAdapter: GitHubCommitListAdapter,
                                                    private val commitListProvider : CommitListProvider) : ViewModel(), LifecycleObserver {
    ...
    private fun setItemAdapter(commitListResponse: List<CommitListResponse>) {
        commitListResponse.forEach {
            ...
            commitItemList.add(GitHubCommitListItemViewModel(it.commit?.author?.name, it.sha, it.commit?.message, it.author.avatarUrl))
        }
